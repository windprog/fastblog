...
def render_sidebar(request):
    ...
    recent_comments = Comments.objects.select_related('comment_post').filter(comment_approved='1',
                                                                             comment_post__post_status='publish',
                                                                             comment_post__post_type='post').order_by(
        '-comment_date')[:7]
    #recent_comments=''
    #return test(request,{'test':recent_comments})

    recent_posts = Posts.objects.filter(post_status='publish', post_type='post').only('id', 'post_title').order_by(
        '-post_date')[:7]
    #categories=TermTaxonomy.objects.select_related('term').filter(taxonomy='category',count__gt=0).order_by('terms')
    categories = Terms.objects.select_related('term').filter(termtaxonomy__taxonomy='category',
                                                             termtaxonomy__count__gt=0).order_by('name')
    ...
    archives = []
    if engine == 'sqlite':
        archives = Posts.objects.filter(post_status='publish', post_type='post').extra(
            select={'year': "strftime('%Y',post_date)", 'month': "strftime('%m',post_date)"}).values('year',
                                                                                                     'month').annotate(
            Count('id')).order_by('-year', '-month')  #.order_by('-post_date') a bug?? wat hell
    elif engine == 'mysql':
        archives = Posts.objects.filter(post_status='publish', post_type='post').extra(
            select={'year': 'year(post_date)', 'month': 'month(post_date)'}).values('year', 'month').annotate(
            Count('id')).order_by('-post_date')
    else:
        pass;
...
def comment(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            comment = Comments(comment_post=p, comment_approved='0', comment_author=comment_author,
                               comment_parent=comment_parent, comment_content=comment_content,
                               comment_author_email=comment_email, comment_author_url=comment_url,
                               comment_author_ip=comment_author_ip, comment_agent=comment_agent)
            ...
            if (p.post_type == 'post'):
                ...
                return HttpResponseRedirect(
                    '/blog/article/' + comment_post_id + '#comment-%s' % comment.comment_id)  #防止重复提交
...
def search(request):
    ...
    posts_list = []
    if s != None:
        posts_list = Posts.objects.all().filter(post_status='publish', post_type='post').filter(
            Q(post_title__contains=s) | Q(post_content__contains=s))
...
def cat(request, num='1'):
    ...
    posts_list = Posts.objects.select_related('termrelationship').filter(termrelationships__term_taxonomy__term_id=num,
                                                                         post_type='post')
...
def test(request):
    ...
    cats = TermRelationships.objects.select_related('term_taxonomy__term').filter(
        term_taxonomy__taxonomy__in=('link_category',), term_taxonomy__count__gt=0)
    all_links = {}
    for l in links:
        for c in cats:
...
def archive(request, year='2014', month='01'):
    posts_list = Posts.objects.all().filter(post_status='publish', post_type='post', post_date__year=year,
...
def render_contents(posts, cat='', more=None):
    cats = TermRelationships.objects.select_related('term_taxonomy__term').filter(
        term_taxonomy__taxonomy__in=('category', 'post_tag', 'post_format'), object_id__in=[p.id for p in posts])
    #cats=Terms.objects.select_related('termtaxonomy').select_related('termrelationships').filter(termtaxonomy__taxonomy__in=('category', 'post_tag', 'post_format'))
    cat_terms = {}
    for cat in cats:
...
def render_article(request, post_id, contexts=None):
    ...
    if comment_author != None and comment_email != None:
        comments = Comments.objects.filter(Q(comment_post_id=post_id), Q(comment_approved='1') | (
        Q(comment_author=comment_author) & Q(comment_author_email=comment_email) & Q(comment_approved='0'))).order_by(
            'comment_date')
    else:
